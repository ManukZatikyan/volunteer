...
export default function CoursesAndActivities() {
  const t = useTranslations("coursesAndActivities");
  const messages = useMessages();
  const coursesAndActivitiesMessages = messages.coursesAndActivities as any;
    ...
    <div className="flex flex-col">
      ...
      <div className="md:px-4 xl:px-30 px-6 pt-12 pb-12">
        <div className="flex flex-col gap-6">
          {(coursesAndActivitiesMessages?.descriptionItems || []).map((item: any, index: number) => (
            <ContentCard
              key={index}
              title={item.heading}
