...
export default function UpcomingEvents() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const response = await fetch(`/api/content/upcomingEvents?locale=${locale}`);
        if (response.ok) {
          const data = await response.json();
          setContent(data.content);
        }
      } catch (error) {
        console.error("Error fetching upcomingEvents content:", error);
      } finally {
        stopLoading();
      }
    };
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="md:px-4 xl:px-30 px-6 pb-12 flex justify-center">
        <Button variant="orange" onClick={() => router.push("/registration?pageKey=upcomingEvents")}>
