...
export default function PRAndPartnership() {
  const t = useTranslations("prAndPartnership");
  const messages = useMessages();
  const prAndPartnershipMessages = messages.prAndPartnership as any;
      ...
      <div className="px-6 md:px-10 xl:px-30 pt-12">
        <div className="flex flex-col gap-6">
          ...
          <MarketingProfileCard
            name={prAndPartnershipMessages?.headOfDepartment?.name || ""}
            description={prAndPartnershipMessages?.headOfDepartment?.description || ""}
            imageSrc={prAndPartnershipMessages?.headOfDepartment?.imageSrc || "/user.png"}
            imageAlt={prAndPartnershipMessages?.headOfDepartment?.imageAlt || ""}
            imagePosition={prAndPartnershipMessages?.headOfDepartment?.imagePosition || "left"}
            socialLinks={prAndPartnershipMessages?.headOfDepartment?.socialLinks || []}
          />
        </div>
      </div>
      <div className="px-6 md:px-10 xl:px-30 py-12">
        <div className="flex flex-col gap-6 md:gap-8 xl:gap-12">
          ...
          <div className="flex flex-col items-center lg:grid lg:grid-cols-2 lg:place-items-center gap-6 lg:gap-y-[48px]">
            {(prAndPartnershipMessages?.members?.items || []).map((member: any, index: number) => (
              <MarketingProfileCard
                key={index}
                name={member.name}
