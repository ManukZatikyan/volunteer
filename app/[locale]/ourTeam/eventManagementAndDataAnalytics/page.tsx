...
export default function EventManagementAndDataAnalytics() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        // Note: 'eventManagementAndDataAnalytics' might not be a valid pageKey, using 'ourTeam' as fallback
        const response = await fetch(`/api/content/ourTeam?locale=${locale}`);
        if (response.ok) {
          const data = await response.json();
          // Extract eventManagementAndDataAnalytics content from ourTeam response
          setContent(data.content?.eventManagementAndDataAnalytics || data.content);
        }
      } catch (error) {
        console.error("Error fetching eventManagementAndDataAnalytics content:", error);
      } finally {
        stopLoading();
      }
    };
