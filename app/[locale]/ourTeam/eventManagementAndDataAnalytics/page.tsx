...
export default function EventManagementAndDataAnalytics() {
  const t = useTranslations("eventManagementAndDataAnalytics");
  const messages = useMessages();
  const eventManagementMessages = messages.eventManagementAndDataAnalytics as any;
    ...
    <div className="flex flex-col">
      <section className="relative w-full h-[246px] sm:h-[400px] flex items-end">
        <div className="absolute inset-0">
          <Image
            src="/image.png"
            alt="Event Management & Data Analytics"
            fill
      ...
      <div className="px-6 md:px-10 xl:px-30 pt-12">
        <div className="flex flex-col gap-6">
          ...
          <MarketingProfileCard
            name={eventManagementMessages?.headOfDepartment?.name || ""}
            description={eventManagementMessages?.headOfDepartment?.description || ""}
            imageSrc={eventManagementMessages?.headOfDepartment?.imageSrc || "/user.png"}
            imageAlt={eventManagementMessages?.headOfDepartment?.imageAlt || ""}
            imagePosition={eventManagementMessages?.headOfDepartment?.imagePosition || "left"}
            socialLinks={eventManagementMessages?.headOfDepartment?.socialLinks || []}
          />
        </div>
      </div>
      <div className="px-6 md:px-10 xl:px-30 py-12">
        <div className="flex flex-col gap-6 md:gap-8 xl:gap-12">
          ...
          <div className="flex flex-col items-center lg:grid lg:grid-cols-2 lg:place-items-center gap-6 lg:gap-y-[48px]">
            {(eventManagementMessages?.members?.items || []).map((member: any, index: number) => (
              <MarketingProfileCard
                key={index}
                name={member.name}
